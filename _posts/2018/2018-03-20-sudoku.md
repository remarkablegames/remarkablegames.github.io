...
Many other algorithms used backtracking to solve the puzzle. The algorithm I hacked together, on the other hand, attempted to generate each row randomly and checked if the solution was valid.

My algorithm wasn't perfect because sometimes an infinite loop would occur. Moreover, it could generate puzzles that aren't unique (has more than 1 solution).
